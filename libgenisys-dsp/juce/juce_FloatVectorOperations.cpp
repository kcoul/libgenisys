...
constexpr std::enable_if_t<exp == 0, T> ipow(T) noexcept
{
    return (T)1;
}

template <int exp, typename T>
constexpr std::enable_if_t<exp == 1, T> ipow(T a) noexcept
{
    return a;
}

template <int exp, typename T>
constexpr std::enable_if_t<exp == 2, T> ipow(T a) noexcept
{
    return a * a;
}

template <int exp, typename T>
constexpr std::enable_if_t<exp == 3, T> ipow(T a) noexcept
{
    return ipow<2>(a) * a;
}

template <int exp, typename T>
constexpr std::enable_if_t<exp == 4, T> ipow(T a) noexcept
{
...
template <int exp, typename T>
constexpr std::enable_if_t<exp == 5, T> ipow(T a) noexcept
{
    return ipow<4>(a) * a;
}

template <int exp, typename T>
constexpr std::enable_if_t<exp == 6, T> ipow(T a) noexcept
{
    const auto b = ipow<2>(a);
    return ipow<3>(b);
}
...
template <int exp, typename T>
constexpr std::enable_if_t<exp == 7, T> ipow(T a) noexcept
{
    return ipow<6>(a) * a;
}

template <int exp, typename T>
constexpr std::enable_if_t<exp == 8, T> ipow(T a) noexcept
{
...
template <int exp, typename T>
constexpr std::enable_if_t<exp == 9, T> ipow(T a) noexcept
{
...
template <int exp, typename T>
constexpr std::enable_if_t<exp == 10, T> ipow(T a) noexcept
{
...
template <int exp, typename T>
constexpr std::enable_if_t<exp == 11, T> ipow(T a) noexcept
{
    return ipow<10>(a) * a;
}

template <int exp, typename T>
constexpr std::enable_if_t<exp == 12, T> ipow(T a) noexcept
{
    const auto d = ipow<4>(a);
    return ipow<3>(d);
}
...
template <int exp, typename T>
constexpr std::enable_if_t<exp == 13, T> ipow(T a) noexcept
{
    return ipow<12>(a) * a;
}

template <int exp, typename T>
constexpr std::enable_if_t<exp == 14, T> ipow(T a) noexcept
{
...
template <int exp, typename T>
constexpr std::enable_if_t<exp == 15, T> ipow(T a) noexcept
{
    const auto e = ipow<5>(a);
    return ipow<3>(e);
}
...
template <int exp, typename T>
constexpr std::enable_if_t<exp == 16, T> ipow(T a) noexcept
{
...
template <int exp, typename T>
constexpr std::enable_if_t<(exp > 16) && exp % 2 == 0, T> ipow(T a) noexcept
{
...
#define JUCE_INCREMENT_DEST dest += (16 / sizeof(*dest));
    ...
    static bool isAligned(const void* p) noexcept { return (((pointer_sized_int)p) & 15) == 0; }
    ...
    struct BasicOps32
    {
        ...
        static forcedinline ParallelType bit_not(ParallelType a, ParallelType b) noexcept
        {
...
#define JUCE_PERFORM_VEC_OP_DEST(normalOp, vecOp, locals, setupOp)                                              \
    ...
    setupOp if (FloatVectorHelpers::isAligned(dest))                                                            \
        JUCE_VEC_LOOP(vecOp, dummy, Mode::loadA, Mode::storeA, locals, JUCE_INCREMENT_DEST) else JUCE_VEC_LOOP( \
            vecOp, dummy, Mode::loadU, Mode::storeU, locals, JUCE_INCREMENT_DEST) JUCE_FINISH_VEC_OP(normalOp)
...
#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(normalOp, vecOp, locals, increment, setupOp)       \
        ...
        {                                                                                          \
            if (FloatVectorHelpers::isAligned(src2))                                               \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                          \
                    vecOp, Mode::loadA, Mode::loadA, Mode::loadA, Mode::storeA, locals, increment) \
            else                                                                                   \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                          \
                    vecOp, Mode::loadA, Mode::loadU, Mode::loadA, Mode::storeA, locals, increment) \
        }                                                                                          \
        else                                                                                       \
        {                                                                                          \
            if (FloatVectorHelpers::isAligned(src2))                                               \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                          \
                    vecOp, Mode::loadU, Mode::loadA, Mode::loadA, Mode::storeA, locals, increment) \
            else                                                                                   \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                          \
                    vecOp, Mode::loadU, Mode::loadU, Mode::loadA, Mode::storeA, locals, increment) \
        }                                                                                          \
    }                                                                                              \
    else                                                                                           \
    {                                                                                              \
        ...
        {                                                                                          \
            if (FloatVectorHelpers::isAligned(src2))                                               \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                          \
                    vecOp, Mode::loadA, Mode::loadA, Mode::loadU, Mode::storeU, locals, increment) \
            else                                                                                   \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                          \
                    vecOp, Mode::loadA, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
        }                                                                                          \
        else                                                                                       \
        {                                                                                          \
            if (FloatVectorHelpers::isAligned(src2))                                               \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                          \
                    vecOp, Mode::loadU, Mode::loadA, Mode::loadU, Mode::storeU, locals, increment) \
            else                                                                                   \
                JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                          \
                    vecOp, Mode::loadU, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
        }                                                                                          \
    }                                                                                              \
    JUCE_FINISH_VEC_OP(normalOp)
    ...
    struct BasicOps32
    {
        ...
        static forcedinline ParallelType bit_and(ParallelType a, ParallelType b) noexcept
        {
        ...
        static forcedinline ParallelType bit_or(ParallelType a, ParallelType b) noexcept
        {
            return toflt(vorrq_u32(toint(a), toint(b)));
        }
        static forcedinline ParallelType bit_xor(ParallelType a, ParallelType b) noexcept
        {
...
#define JUCE_PERFORM_VEC_OP_DEST(normalOp, vecOp, locals, setupOp)                              \
    JUCE_BEGIN_VEC_OP                                                                           \
    setupOp JUCE_VEC_LOOP(vecOp, dummy, Mode::loadU, Mode::storeU, locals, JUCE_INCREMENT_DEST) \
        JUCE_FINISH_VEC_OP(normalOp)

#define JUCE_PERFORM_VEC_OP_SRC_DEST(normalOp, vecOp, locals, increment, setupOp) \
    JUCE_BEGIN_VEC_OP                                                             \
    setupOp JUCE_VEC_LOOP(vecOp, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) JUCE_FINISH_VEC_OP(normalOp)

#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(normalOp, vecOp, locals, increment, setupOp)                 \
    JUCE_BEGIN_VEC_OP                                                                                   \
    setupOp JUCE_VEC_LOOP_TWO_SOURCES(vecOp, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) \
        JUCE_FINISH_VEC_OP(normalOp)

#define JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(normalOp, vecOp, locals, increment, setupOp) \
    JUCE_BEGIN_VEC_OP                                                                        \
    setupOp JUCE_VEC_LOOP_TWO_SOURCES_WITH_DEST_LOAD(                                        \
        vecOp, Mode::loadU, Mode::loadU, Mode::loadU, Mode::storeU, locals, increment) JUCE_FINISH_VEC_OP(normalOp)
    ...
    struct MinMax
    {
        ...
        static Type findMinOrMax(const Type* src, int num, const bool isMinimum) noexcept
        {
            ...
            if (numLongOps > 1)
            {
                ...
                Type result = isMinimum ? Mode::min(val) : Mode::max(val);

                num &= (Mode::numParallel - 1);
                src += Mode::numParallel;

                for (int i = 0; i < num; ++i)
                    result = isMinimum ? jmin(result, src[i]) : jmax(result, src[i]);
        ...
        static Range<Type> findMinAndMax(const Type* src, int num) noexcept
        {
            ...
            if (numLongOps > 1)
            {
                ...
                Range<Type> result(Mode::min(mn), Mode::max(mx));
...
void FloatVectorOperations::clear(double* dest, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::fill(float* dest, float valueToFill, int num) noexcept
{
    ...
    JUCE_PERFORM_VEC_OP_DEST(
        dest[i] = valueToFill, val, JUCE_LOAD_NONE, const Mode::ParallelType val = Mode::load1(valueToFill);)
#endif
}

void FloatVectorOperations::fill(double* dest, double valueToFill, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::copy(float* dest, const float* src, int num) noexcept
{
    memcpy(dest, src, (size_t)num * sizeof(float));
}

void FloatVectorOperations::copy(double* dest, const double* src, int num) noexcept
{
...
void FloatVectorOperations::copyWithMultiply(double* dest, const double* src, double multiplier, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::add(double* dest, double amount, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_DEST(dest[i] += amount,
                             Mode::add(d, amountToAdd),
                             JUCE_LOAD_DEST,
...
void FloatVectorOperations::add(float* dest, const float* src, float amount, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::add(double* dest, const double* src, double amount, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::add(double* dest, const double* src, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::add(float* dest, const float* src1, const float* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
    vDSP_vadd(src1, 1, src2, 1, dest, 1, (vDSP_Length)num);
#else
    JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(
        dest[i] = src1[i] + src2[i], Mode::add(s1, s2), JUCE_LOAD_SRC1_SRC2, JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

void FloatVectorOperations::add(double* dest, const double* src1, const double* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::subtract(float* dest, const float* src, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::subtract(double* dest, const double* src, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::subtract(float* dest, const float* src1, const float* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
    vDSP_vsub(src2, 1, src1, 1, dest, 1, (vDSP_Length)num);
#else
    JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(
        dest[i] = src1[i] - src2[i], Mode::sub(s1, s2), JUCE_LOAD_SRC1_SRC2, JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

void FloatVectorOperations::subtract(double* dest, const double* src1, const double* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::addWithMultiply(double* dest, const double* src, double multiplier, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::addWithMultiply(float* dest, const float* src1, const float* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::addWithMultiply(double* dest, const double* src1, const double* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::subtractWithMultiply(float* dest, const float* src, float multiplier, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] -= src[i] * multiplier,
...
void FloatVectorOperations::subtractWithMultiply(double* dest, const double* src, double multiplier, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] -= src[i] * multiplier,
                                 Mode::sub(d, Mode::mul(mult, s)),
                                 JUCE_LOAD_SRC_DEST,
...
void FloatVectorOperations::subtractWithMultiply(float* dest, const float* src1, const float* src2, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(dest[i] -= src1[i] * src2[i],
...
void FloatVectorOperations::subtractWithMultiply(double* dest, const double* src1, const double* src2, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST_DEST(dest[i] -= src1[i] * src2[i],
                                            Mode::sub(d, Mode::mul(s1, s2)),
                                            JUCE_LOAD_SRC1_SRC2_DEST,
...
void FloatVectorOperations::multiply(double* dest, const double* src, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::multiply(float* dest, const float* src1, const float* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
    vDSP_vmul(src1, 1, src2, 1, dest, 1, (vDSP_Length)num);
#else
    JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(
        dest[i] = src1[i] * src2[i], Mode::mul(s1, s2), JUCE_LOAD_SRC1_SRC2, JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

void FloatVectorOperations::multiply(double* dest, const double* src1, const double* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::multiply(double* dest, double multiplier, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::multiply(float* dest, const float* src, float multiplier, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = src[i] * multiplier,
                                 Mode::mul(mult, s),
                                 JUCE_LOAD_SRC,
...
void FloatVectorOperations::multiply(double* dest, const double* src, double multiplier, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = src[i] * multiplier,
                                 Mode::mul(mult, s),
                                 JUCE_LOAD_SRC,
...
void FloatVectorOperations::min(float* dest, const float* src, float comp, int num) noexcept
{
...
void FloatVectorOperations::min(double* dest, const double* src, double comp, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = jmin(src[i], comp),
                                 Mode::min(s, cmp),
                                 JUCE_LOAD_SRC,
...
void FloatVectorOperations::min(float* dest, const float* src1, const float* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
    vDSP_vmin((float*)src1, 1, (float*)src2, 1, dest, 1, (vDSP_Length)num);
#else
    JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(
        dest[i] = jmin(src1[i], src2[i]), Mode::min(s1, s2), JUCE_LOAD_SRC1_SRC2, JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

void FloatVectorOperations::min(double* dest, const double* src1, const double* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::max(float* dest, const float* src, float comp, int num) noexcept
{
...
void FloatVectorOperations::max(double* dest, const double* src, double comp, int num) noexcept
{
    JUCE_PERFORM_VEC_OP_SRC_DEST(dest[i] = jmax(src[i], comp),
                                 Mode::max(s, cmp),
                                 JUCE_LOAD_SRC,
...
void FloatVectorOperations::max(float* dest, const float* src1, const float* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
    vDSP_vmax((float*)src1, 1, (float*)src2, 1, dest, 1, (vDSP_Length)num);
#else
    JUCE_PERFORM_VEC_OP_SRC1_SRC2_DEST(
        dest[i] = jmax(src1[i], src2[i]), Mode::max(s1, s2), JUCE_LOAD_SRC1_SRC2, JUCE_INCREMENT_SRC1_SRC2_DEST, )
#endif
}

void FloatVectorOperations::max(double* dest, const double* src1, const double* src2, int num) noexcept
{
#if JUCE_USE_VDSP_FRAMEWORK
...
void FloatVectorOperations::clip(float* dest, const float* src, float low, float high, int num) noexcept
{
    assert(high >= low);
...
void FloatVectorOperations::clip(double* dest, const double* src, double low, double high, int num) noexcept
{
    assert(high >= low);
...
Range<float> FloatVectorOperations::findMinAndMax(const float* src, int num) noexcept
{
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
...
Range<double> FloatVectorOperations::findMinAndMax(const double* src, int num) noexcept
{
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
...
float FloatVectorOperations::findMinimum(const float* src, int num) noexcept
{
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
...
double FloatVectorOperations::findMinimum(const double* src, int num) noexcept
{
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
...
float FloatVectorOperations::findMaximum(const float* src, int num) noexcept
{
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
...
double FloatVectorOperations::findMaximum(const double* src, int num) noexcept
{
#if JUCE_USE_SSE_INTRINSICS || JUCE_USE_ARM_NEON
...
intptr_t FloatVectorOperations::getFpStatusRegister() noexcept
{
    intptr_t fpsr = 0;
    ...
    assert(false); // No support for getting the floating point status register for your platform
#endif
#endif
...
void FloatVectorOperations::setFpStatusRegister(intptr_t fpsr) noexcept
{
#if JUCE_INTEL && JUCE_USE_SSE_INTRINSICS
...
void FloatVectorOperations::enableFlushToZeroMode(bool shouldEnable) noexcept
{
...
#else
#if !(defined(JUCE_INTEL) || defined(JUCE_ARM))
    assert(false); // No support for flush to zero mode on your platform
#endif
    ignoreUnused(shouldEnable);
#endif
}

void FloatVectorOperations::disableDenormalisedNumberSupport(bool shouldDisable) noexcept
{
#if JUCE_USE_SSE_INTRINSICS || (JUCE_USE_ARM_NEON || defined(__arm64__) || defined(__aarch64__))
#if JUCE_USE_SSE_INTRINSICS
...
#if !(defined(JUCE_INTEL) || defined(JUCE_ARM))
    assert(false); // No support for disable denormals mode on your platform
#endif
#endif
}

bool FloatVectorOperations::areDenormalsDisabled() noexcept
{
#if JUCE_USE_SSE_INTRINSICS || (JUCE_USE_ARM_NEON || defined(__arm64__) || defined(__aarch64__))
#if JUCE_USE_SSE_INTRINSICS
    intptr_t mask = 0x8040;
...
namespace detail
{
    ...
    template <typename T,
              typename ScalarOp,
              typename VecOp,
              typename LoadOp1Type,
              typename LoadOp2Type,
              typename StoreOpType>
    void binaryOp(T* dest,
                  const T* src1,
                  const T* src2,
                  int numValues,
                  ScalarOp&& scalarOp,
    ...
    template <typename T, typename ScalarOp, typename VecOp, typename VecReduceOp>
    T reduce(const T* src1,
             const T* src2,
             int numValues,
             T init,
             ScalarOp&& scalarOp,
             VecOp&& vecOp,
             VecReduceOp&& vecReduceOp)
    {
        constexpr auto vecSize = (int)xsimd::batch<T>::size;
        auto numVecOps = numValues / vecSize;
...
#else
    detail::binaryOp(dest, dividend, divisor, numValues, [](auto num, auto den) { return num / den; });
#endif
}
...
#else
    detail::unaryOp(dest, divisor, numValues, [dividend](auto x) { return dividend / x; });
#endif
}
...
// TODO (JC): Figure out why vDSP_sve is failing unit tests in CI?
// TODO (KC): vDSP_sve is producing equivalent results to detail::reduce, reinstating
float FloatVectorOperations::accumulate(const float* src, int numValues) noexcept
{
    ...
    return result;
#else
    return detail::reduce(src, numValues, 0.0f, [](auto prev, auto next) { return prev + next; });
#endif
}
...
double FloatVectorOperations::accumulate(const double* src, int numValues) noexcept
{
    ...
    return result;
#else
    return detail::reduce(src, numValues, 0.0, [](auto prev, auto next) { return prev + next; });
#endif
}
...
float FloatVectorOperations::innerProduct(const float* src1, const float* src2, int numValues) noexcept
{
    ...
    return detail::reduce(
        src1, src2, numValues, 0.0f, [](auto prev, auto next1, auto next2) { return prev + next1 * next2; });
#endif
}
...
#else
    return detail::reduce(
        src1, src2, numValues, 0.0, [](auto prev, auto next1, auto next2) { return prev + next1 * next2; });
#endif
}
...
float FloatVectorOperations::computeRMS(const float* src, int numValues) noexcept
{
    ...
    const auto squareSum =
        detail::reduce(src, numValues, 0.0f, [](auto prev, auto next) { return prev + next * next; });
...
double FloatVectorOperations::computeRMS(const double* src, int numValues) noexcept
{
    ...
    return result;
#else
    const auto squareSum = detail::reduce(src, numValues, 0.0, [](auto prev, auto next) { return prev + next * next; });
...
{
public:
    FloatVectorOperationsTests() : UnitTest("FloatVectorOperations", UnitTestCategories::audio) {}

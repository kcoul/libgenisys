...
AbstractFifo::AbstractFifo(int capacity) noexcept : bufferSize(capacity) { assert(bufferSize > 0); }
...
//==============================================================================
void AbstractFifo::prepareToWrite(int numToWrite,
                                  int& startIndex1,
                                  int& blockSize1,
                                  int& startIndex2,
                                  int& blockSize2) const noexcept
{
    auto vs = validStart.get();
    auto ve = validEnd.get();
    ...
    else
    {
        ...
        blockSize2 = numToWrite <= 0 ? 0 : std::min(numToWrite, vs);
    }
}

void AbstractFifo::finishedWrite(int numWritten) noexcept
{
    assert(numWritten >= 0 && numWritten < bufferSize);
...
void AbstractFifo::prepareToRead(int numWanted,
                                 int& startIndex1,
                                 int& blockSize1,
                                 int& startIndex2,
                                 int& blockSize2) const noexcept
{
    ...
    else
    {
        ...
        blockSize2 = numWanted <= 0 ? 0 : std::min(numWanted, ve);
    }
}

void AbstractFifo::finishedRead(int numRead) noexcept
{
    assert(numRead >= 0 && numRead <= bufferSize);
...
template <AbstractFifo::ReadOrWrite mode>
AbstractFifo::ScopedReadWrite<mode>& AbstractFifo::ScopedReadWrite<mode>::operator=(ScopedReadWrite&& other) noexcept
{
    swap(other);
    return *this;
}
...
{
public:
    AbstractFifoTests() : UnitTest("Abstract Fifo", UnitTestCategories::containers) {}

    struct WriteThread : public Thread
    {
        WriteThread(AbstractFifo& f, int* b, Random rng) : Thread("fifo writer"), fifo(f), buffer(b), random(rng)
        {
            startThread();
        }
    ...
    void runTest() override
    {
        ...
        for (int count = 100000; --count >= 0;)
        {
            ...
            if (!(reader.blockSize1 >= 0 && reader.blockSize2 >= 0)
                && (reader.blockSize1 == 0 || (reader.startIndex1 >= 0 && reader.startIndex1 < fifo.getTotalSize()))
                && (reader.blockSize2 == 0 || (reader.startIndex2 >= 0 && reader.startIndex2 < fifo.getTotalSize())))
            {
            ...
            reader.forEach([&failed, &buffer, &n](int index) { failed = (buffer[index] != n++) || failed; });

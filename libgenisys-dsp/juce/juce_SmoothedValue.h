...
private:
    //==============================================================================
    FloatType getNextSmoothedValue() noexcept { return static_cast<SmoothedValueType*>(this)->getNextValue(); }
...
public:
    ...
    */
    [[deprecated("Use setTargetValue and setCurrentAndTargetValue instead.")]] void
        setValue(FloatType newValue, bool force = false) noexcept
    {
        if (force)
        {
            this->setCurrentAndTargetValue(newValue);
...
private:
    ...
    using MultiplicativeVoid =
        typename std::enable_if<std::is_same<T, ValueSmoothingTypes::Multiplicative>::value, void>::type;
    ...
    {
        step = std::exp((std::log(std::abs(this->target)) - std::log(std::abs(this->currentValue)))
...
{
public:
    CommonSmoothedValueTests() : UnitTest("CommonSmoothedValueTests", UnitTestCategories::smoothedValues) {}
    ...
    void runTest() override
    {
        ...
        beginTest("Sample rate");
        {
            ...
            {
                svTime.skip(1);
                expectWithinAbsoluteError(svSamples.getNextValue(), svTime.getNextValue(), 1.0e-7f);
            }
        }
        ...
        beginTest("Block processing");
        {
            ...
            expectWithinAbsoluteError(referenceData.getSample(0, 11), sv.getTargetValue(), 2.0e-7f);
            ...
            auto compareData = [this](const AudioBuffer<float>& test, const AudioBuffer<float>& reference)
            {
                for (int i = 0; i < test.getNumSamples(); ++i)
                    expectWithinAbsoluteError(test.getSample(0, i), reference.getSample(0, i), 2.0e-7f);
            };
            ...
            sv.setCurrentAndTargetValue(1.0f);
            sv.setTargetValue(2.0f);
            sv.applyGain(destData.getWritePointer(0), testData.getReadPointer(0), numSamples);
        ...
        beginTest("Negative");
        {
            ...
            std::vector<std::pair<float, float>> ranges = { { -1.0f, -2.0f }, { -100.0f, -3.0f } };

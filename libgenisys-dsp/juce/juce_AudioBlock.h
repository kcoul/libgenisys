...
*/
    ...
    private:
        ...
        using MayUseConvertingConstructor =
            std::enable_if_t<std::is_same<std::remove_const_t<SampleType>, std::remove_const_t<OtherSampleType>>::value
                                 && std::is_const<SampleType>::value && !std::is_const<OtherSampleType>::value,
                             int>;
    ...
    */
        AudioBlock(HeapBlock<char>& heapBlockToUseForAllocation,
            ...
            : numChannels(static_cast<ChannelCountType>(numberOfChannels)), numSamples(numberOfSamples)
        {
    ...
    */
        ...
        {
            return std::equal(channels, channels + numChannels, other.channels, other.channels + other.numChannels)
                   && startSample == other.startSample && numSamples == other.numSamples;
        }
    ...
    */
        ...
        in the AudioBuffer's units by a factor of SIMDRegister<SampleType>::SIMDNumElements.
    */
        void copyTo(AudioBuffer<typename std::remove_const<NumericType>::type>& dst,
                    size_t srcPos = 0,
                    size_t dstPos = 0,
                    size_t numElements = std::numeric_limits<size_t>::max()) const
        {
    ...
    */
        ...
        const AudioBlock&
            move(size_t srcPos, size_t dstPos, size_t numElements = std::numeric_limits<size_t>::max()) const noexcept
        {
            moveInternal(srcPos, dstPos, numElements);
            return *this;
        }
    ...
    */
        ...
        template <typename OtherSampleType>
        const AudioBlock& JUCE_VECTOR_CALLTYPE replaceWithSumOf(AudioBlock<OtherSampleType> src,
                                                                NumericType value) const noexcept
        {
            replaceWithSumOfInternal(src, value);
            return *this;
        }
        ...
        template <typename Src1SampleType, typename Src2SampleType>
        const AudioBlock& replaceWithSumOf(AudioBlock<Src1SampleType> src1,
                                           AudioBlock<Src2SampleType> src2) const noexcept
        {
            replaceWithSumOfInternal(src1, src2);
            return *this;
        }
        ...
        template <typename OtherSampleType>
        AudioBlock& JUCE_VECTOR_CALLTYPE replaceWithDifferenceOf(AudioBlock<OtherSampleType> src,
        ...
        template <typename OtherSampleType>
        const AudioBlock& JUCE_VECTOR_CALLTYPE replaceWithDifferenceOf(AudioBlock<OtherSampleType> src,
                                                                       NumericType value) const noexcept
        {
            replaceWithDifferenceOfInternal(src, value);
            return *this;
        ...
        template <typename Src1SampleType, typename Src2SampleType>
        const AudioBlock& replaceWithDifferenceOf(AudioBlock<Src1SampleType> src1,
                                                  AudioBlock<Src2SampleType> src2) const noexcept
        {
            replaceWithDifferenceOfInternal(src1, src2);
            return *this;
        ...
        template <typename OtherSampleType>
        AudioBlock& JUCE_VECTOR_CALLTYPE replaceWithProductOf(AudioBlock<OtherSampleType> src,
                                                              NumericType value) noexcept
        {
        ...
        template <typename OtherSampleType>
        const AudioBlock& JUCE_VECTOR_CALLTYPE replaceWithProductOf(AudioBlock<OtherSampleType> src,
                                                                    NumericType value) const noexcept
        {
            replaceWithProductOfInternal(src, value);
            return *this;
        ...
        template <typename Src1SampleType, typename Src2SampleType>
        const AudioBlock& replaceWithProductOf(AudioBlock<Src1SampleType> src1,
                                               AudioBlock<Src2SampleType> src2) const noexcept
        {
            replaceWithProductOfInternal(src1, src2);
            return *this;
        ...
        AudioBlock& replaceWithProductOf(AudioBlock<BlockSampleType> src,
                                         SmoothedValue<SmootherSampleType, SmoothingType>& value) noexcept
        {
            replaceWithProductOfInternal(src, value);
            return *this;
        }
        template <typename BlockSampleType, typename SmootherSampleType, typename SmoothingType>
        const AudioBlock& replaceWithProductOf(AudioBlock<BlockSampleType> src,
                                               SmoothedValue<SmootherSampleType, SmoothingType>& value) const noexcept
        {
            replaceWithProductOfInternal(src, value);
            return *this;
        }
        ...
        template <typename OtherSampleType>
        const AudioBlock& JUCE_VECTOR_CALLTYPE addProductOf(AudioBlock<OtherSampleType> src,
                                                            NumericType factor) const noexcept
        {
            addProductOfInternal(src, factor);
            return *this;
        }
        ...
        template <typename Src1SampleType, typename Src2SampleType>
        const AudioBlock& replaceWithMinOf(AudioBlock<Src1SampleType> src1,
                                           AudioBlock<Src2SampleType> src2) const noexcept
        {
            replaceWithMinOfInternal(src1, src2);
            return *this;
        ...
        template <typename Src1SampleType, typename Src2SampleType>
        const AudioBlock& replaceWithMaxOf(AudioBlock<Src1SampleType> src1,
                                           AudioBlock<Src2SampleType> src2) const noexcept
        {
            replaceWithMaxOfInternal(src1, src2);
            return *this;
    ...
    */
        ...
        static void
            process(AudioBlock<Src1SampleType> inBlock, AudioBlock<Src2SampleType> outBlock, FunctionType&& function)
        {
    ...
    private:
        ...
        template <typename OtherNumericType>
        void copyFromInternal(const AudioBuffer<OtherNumericType>& src,
                              size_t srcPos,
                              size_t dstPos,
        ...
        void moveInternal(size_t srcPos,
                          size_t dstPos,
                          size_t numElements = std::numeric_limits<size_t>::max()) const noexcept
        {
            ...
            if (len != 0)
                for (size_t ch = 0; ch < numChannels; ++ch)
                    ::memmove(getChannelPointer(ch) + dstPos, getChannelPointer(ch) + srcPos, len);
        }
        ...
        template <typename OtherSampleType>
        void JUCE_VECTOR_CALLTYPE replaceWithSumOfInternal(AudioBlock<OtherSampleType> src,
                                                           NumericType value) const noexcept
        {
            jassert(numChannels == src.numChannels);
            auto n = jmin(numSamples, src.numSamples) * sizeFactor;
        ...
        template <typename OtherSampleType>
        void JUCE_VECTOR_CALLTYPE replaceWithDifferenceOfInternal(AudioBlock<OtherSampleType> src,
                                                                  NumericType value) const noexcept
        {
        ...
        template <typename Src1SampleType, typename Src2SampleType>
        void replaceWithDifferenceOfInternal(AudioBlock<Src1SampleType> src1,
                                             AudioBlock<Src2SampleType> src2) const noexcept
        {
            jassert(numChannels == src1.numChannels && src1.numChannels == src2.numChannels);
            auto n = jmin(numSamples, src1.numSamples, src2.numSamples) * sizeFactor;
            ...
            for (size_t ch = 0; ch < numChannels; ++ch)
                FloatVectorOperations::subtract(
                    getDataPointer(ch), src1.getDataPointer(ch), src2.getDataPointer(ch), n);
        }
        ...
        template <typename OtherSampleType>
        void JUCE_VECTOR_CALLTYPE replaceWithProductOfInternal(AudioBlock<OtherSampleType> src,
                                                               NumericType value) const noexcept
        {
            jassert(numChannels == src.numChannels);
            auto n = jmin(numSamples, src.numSamples) * sizeFactor;
        ...
        template <typename Src1SampleType, typename Src2SampleType>
        void replaceWithProductOfInternal(AudioBlock<Src1SampleType> src1,
                                          AudioBlock<Src2SampleType> src2) const noexcept
        {
            jassert(numChannels == src1.numChannels && src1.numChannels == src2.numChannels);
            auto n = jmin(numSamples, src1.numSamples, src2.numSamples) * sizeFactor;
        ...
        template <typename BlockSampleType, typename SmootherSampleType, typename SmoothingType>
        void replaceWithProductOfInternal(AudioBlock<BlockSampleType> src,
                                          SmoothedValue<SmootherSampleType, SmoothingType>& value) const noexcept
        {
            jassert(numChannels == src.numChannels);
        ...
        //==============================================================================
        template <typename OtherSampleType>
        void JUCE_VECTOR_CALLTYPE addProductOfInternal(AudioBlock<OtherSampleType> src,
                                                       NumericType factor) const noexcept
        {
            jassert(numChannels == src.numChannels);
            auto n = jmin(numSamples, src.numSamples) * sizeFactor;
        ...
        //==============================================================================
        constexpr void negateInternal() const noexcept { multiplyByInternal(static_cast<NumericType>(-1.0)); }

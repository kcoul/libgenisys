...
namespace detail
{
    ...
    */
    ...
    {
        return std::max(alignof(Head), getMaxAlignment(tail...));
    }
#if __linux__
...
public:
    ...
    AudioBuffer() noexcept : channels(static_cast<Type**>(preallocatedChannelSpace)) {}
    ...
    */
    AudioBuffer(int numChannelsToAllocate, int numSamplesToAllocate)
        : numChannels(numChannelsToAllocate), size(numSamplesToAllocate)
    {
        assert(size >= 0 && numChannels >= 0);
        allocateData();
    }
    ...
    */
    AudioBuffer(Type* const* dataToReferTo, int numChannelsToUse, int numSamples)
        : numChannels(numChannelsToUse), size(numSamples)
    {
        assert(dataToReferTo != nullptr);
        assert(numChannelsToUse >= 0 && numSamples >= 0);
    ...
    */
    ...
    {
        assert(dataToReferTo != nullptr);
        assert(numChannelsToUse >= 0 && startSample >= 0 && numSamples >= 0);
    ...
    */
    AudioBuffer(const AudioBuffer& other)
        : numChannels(other.numChannels), size(other.size), allocatedBytes(other.allocatedBytes)
    {
        if (allocatedBytes == 0)
        {
    ...
    */
    ...
    {
        assert(isPositiveAndBelow(channelNumber, numChannels));
    ...
    */
    ...
    {
        assert(isPositiveAndBelow(channelNumber, numChannels));
        assert(isPositiveAndBelow(sampleIndex, size));
    ...
    */
                 ...
                 bool avoidReallocating = false)
    {
        ...
        if (newNumSamples != size || newNumChannels != numChannels)
        {
            ...
            auto newTotalBytes =
                ((size_t)newNumChannels * (size_t)allocatedSamplesPerChannel * sizeof(Type)) + channelListSize + 32;
            ...
            if (keepExistingContent)
            {
                ...
                else
                {
                    ...
                    auto numSamplesToCopy = (size_t)std::min(newNumSamples, size);
                    ...
                    if (!isClear)
                    {
                        auto numChansToCopy = std::min(numChannels, newNumChannels);
    ...
    */
    ...
    {
        assert(dataToReferTo != nullptr);
        assert(newNumChannels >= 0 && newNumSamples >= 0);
    ...
    */
    ...
    {
        assert(isPositiveAndBelow(channel, numChannels));
        assert(startSample >= 0 && numSamples >= 0 && startSample + numSamples <= size);
    ...
    */
    ...
    {
        assert(isPositiveAndBelow(destChannel, numChannels));
        assert(isPositiveAndBelow(destSample, size));
    ...
    */
    ...
    {
        assert(&source != this || sourceChannel != destChannel || sourceStartSample + numSamples <= destStartSample
               || destStartSample + numSamples <= sourceStartSample);
        assert(isPositiveAndBelow(destChannel, numChannels));
        assert(destStartSample >= 0 && numSamples >= 0 && destStartSample + numSamples <= size);
        assert(isPositiveAndBelow(sourceChannel, source.numChannels));
        assert(sourceStartSample >= 0 && sourceStartSample + numSamples <= source.size);
    ...
    */
                  ...
                  int numSamples) noexcept
    {
        assert(&source != this || sourceChannel != destChannel || sourceStartSample + numSamples <= destStartSample
               || destStartSample + numSamples <= sourceStartSample);
        assert(isPositiveAndBelow(destChannel, numChannels));
        assert(destStartSample >= 0 && destStartSample + numSamples <= size);
        assert(isPositiveAndBelow(sourceChannel, source.numChannels));
        assert(sourceStartSample >= 0 && numSamples >= 0 && sourceStartSample + numSamples <= source.size);
...
private:
    ...
    void allocateData()
    {
        ...
        static_assert(
            alignof(Type) <= detail::maxAlignment,
    ...
    void allocateChannels(Type* const* dataToReferTo, int offset)
    {
        ...
        for (int i = 0; i < numChannels; ++i)
        {
            ...
            assert(dataToReferTo[i] != nullptr);

...
{
    assert(sourceRangeMax != sourceRangeMin); // mapping from a range of zero will produce NaN!
...
{
    assert(Type1()
           <= static_cast<Type1>(upperLimit)); // makes no sense to call this if the upper limit is itself below zero..
...
unsigned int truncatePositiveToUnsignedInt(FloatType value) noexcept
{
    ...
    assert(static_cast<FloatType>(value) <= static_cast<FloatType>(std::numeric_limits<unsigned int>::max()));

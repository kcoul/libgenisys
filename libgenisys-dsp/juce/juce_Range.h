...
public:
    ...
    constexpr Range(const ValueType startValue, const ValueType endValue) noexcept
        : start(startValue), end(std::max(startValue, endValue))
    {
    }
    ...
    {
        return position1 < position2 ? Range(position1, position2) : Range(position2, position1);
    }
    ...
    constexpr static Range emptyRange(const ValueType start) noexcept { return Range(start, start); }
    ...
    */
    ...
    /** Returns a range with the same start position as this one, but a different end.
        ...
        will be pushed back to equal the new end point.
    */
    constexpr Range withEnd(const ValueType newEnd) const noexcept { return Range(std::min(start, newEnd), newEnd); }
    ...
    */
    void setLength(const ValueType newLength) noexcept { end = start + jmax(ValueType(), newLength); }

    /** Returns a range with the same start as this one, but a different length.
        Lengths less than zero are treated as zero.
    */
    constexpr Range withLength(const ValueType newLength) const noexcept { return Range(start, start + newLength); }
    ...
    */
    constexpr Range expanded(ValueType amount) const noexcept { return Range(start - amount, end + amount); }
    ...
    */
    constexpr bool contains(const ValueType position) const noexcept { return start <= position && position < end; }

    /** Returns the nearest value to the one supplied, which lies within the range. */
    ValueType clipValue(const ValueType value) const noexcept { return jlimit(start, end, value); }

    /** Returns true if the given range lies entirely inside this range. */
    constexpr bool contains(Range other) const noexcept { return start <= other.start && end >= other.end; }

    /** Returns true if the given range intersects this one. */
    constexpr bool intersects(Range other) const noexcept { return other.start < end && start < other.end; }

    /** Returns the range that is the intersection of the two ranges, or an empty range
        with an undefined start position if they don't overlap. */
    constexpr Range getIntersectionWith(Range other) const noexcept
    {
        return Range(jmax(start, other.start), jmin(end, other.end));
    }

    /** Returns the smallest range that contains both this one and the other one. */
    constexpr Range getUnionWith(Range other) const noexcept
    {
        return Range(jmin(start, other.start), jmax(end, other.end));
    }
    ...
    {
        return Range(jmin(valueToInclude, start), jmax(valueToInclude, end));
    }

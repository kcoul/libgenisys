...
#include "juce/juce_AbstractFifo.h"
#include "juce/juce_AudioSampleBuffer.h"
#include "juce/juce_FloatVectorOperations.h"
#include <assert.h>
...
public:
    ...
    void ensureFreeSpace(int numSamples)
    {
        ...
        if (numSamples > freeSpace)
        {
            ...
            assert(samplesRequired <= getNumReady());
    ...
    bool read(juce::AudioSampleBuffer& dest) { return read(dest, 0, dest.getNumSamples()); }
    ...
    bool readAdding(juce::AudioSampleBuffer& dest) { return readAdding(dest, 0, dest.getNumSamples()); }

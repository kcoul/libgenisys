...
static int linear_vari_process(SRC_PRIVATE* psrc, SRC_DATA* data)
{
    ...
    while (input_index < 1.0 && priv->out_gen < priv->out_count)
    {
        ...
        for (ch = 0; ch < priv->channels; ch++)
        {
            data->data_out[priv->out_gen] =
                (float)(priv->last_value[ch] + input_index * (data->data_in[ch] - priv->last_value[ch]));
            priv->out_gen++;
        };
    ...
    while (priv->out_gen < priv->out_count && priv->in_used + priv->channels * input_index < priv->in_count)
    {
        ...
        for (ch = 0; ch < priv->channels; ch++)
        {
            data->data_out[priv->out_gen] = (float)(data->data_in[priv->in_used - priv->channels + ch]
                                                    + input_index
                                                          * (data->data_in[priv->in_used + ch]
                                                             - data->data_in[priv->in_used - priv->channels + ch]));
            priv->out_gen++;
        };

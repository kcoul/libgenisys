...
static inline increment_t double_to_fp(double x) { return (increment_t)(lrint((x)*FP_ONE)); } /* double_to_fp */

static inline increment_t int_to_fp(int x) { return (((increment_t)(x)) << SHIFT_BITS); } /* int_to_fp */

static inline int fp_to_int(increment_t x) { return (((x) >> SHIFT_BITS)); } /* fp_to_int */
...
static inline double fp_to_double(increment_t x) { return fp_fraction_part(x) * INV_FP_ONE; } /* fp_to_double */
...
int sinc_set_converter(SRC_PRIVATE* psrc, int src_enum)
{
    ...
    */
    ...
    temp_filter.b_len +=
        1; // There is a <= check against samples_in_hand requiring a buffer bigger than the calculation above

    if ((filter = ZERO_ALLOC(
             SINC_FILTER, sizeof(SINC_FILTER) + sizeof(filter->buffer[0]) * (temp_filter.b_len + temp_filter.channels)))
        == NULL)
...
static int sinc_copy(SRC_PRIVATE* from, SRC_PRIVATE* to)
{
    ...
    size_t private_length =
        sizeof(SINC_FILTER) + sizeof(from_filter->buffer[0]) * (from_filter->b_len + from_filter->channels);
...
static inline double calc_output_single(SINC_FILTER* filter, increment_t increment, increment_t start_filter_index)
{
...
static int sinc_mono_vari_process(SRC_PRIVATE* psrc, SRC_DATA* data)
{
    ...
    while (filter->out_gen < filter->out_count)
    {
        ...
        data->data_out[filter->out_gen] =
            (float)((float_increment / filter->index_inc) * calc_output_single(filter, increment, start_filter_index));
        filter->out_gen++;
...
static inline void calc_output_stereo(SINC_FILTER* filter,
...
static inline void calc_output_quad(SINC_FILTER* filter,
                                    increment_t increment,
                                    increment_t start_filter_index,
                                    double scale,
                                    float* output)
...
static inline void calc_output_hex(SINC_FILTER* filter,
                                   increment_t increment,
                                   increment_t start_filter_index,
                                   double scale,
                                   float* output)
...
static inline void calc_output_multi(SINC_FILTER* filter,
                                     increment_t increment,
                                     increment_t start_filter_index,
                                     int channels,
                                     double scale,
...
static int prepare_data(SINC_FILTER* filter, SRC_DATA* data, int half_filter_chan_len)
{
    ...
    if (filter->in_used == filter->in_count && filter->b_end - filter->b_current < 2 * half_filter_chan_len
        && data->end_of_input)
